...
$(function(){
    var classVue=window.sessionStorage.getItem('classVue');
    $("#logo").empty().append(`<a href="/" title="${_title}">溯源管理系统</a>`)
    .css("margin-top","18px")
    .css("margin-left","30px");
    $("#logo a").css("color","ivory").css("font-size","14px");
    if(classVue=="smart-style-5"){
        $("#iform,#diqu-fixed,table").css("background-color","rgba(250, 250, 250, 0)");
    }
    if(classVue == 'ng'){
        classVue = 'smart-style-6 ng'
    }
    $('body').removeClassPrefix("smart-style").addClass(classVue);
    $("html").removeClassPrefix("smart-style").addClass(classVue);
});
